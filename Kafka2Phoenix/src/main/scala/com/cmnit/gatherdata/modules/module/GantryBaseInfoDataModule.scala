...
import com.cmnit.gatherdata.modules.bean.GantryBaseInfoData
...
object GantryBaseInfoDataModule {

  final val logger = Logger.getLogger(GantryBaseInfoDataModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.RM_BASEINFOUPLOAD_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[GantryBaseInfoData])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_GantryBaseInfoData")
        sparkSession.sql("insert into table ods.ods_etc_gantrybaseinfodata partition(year,month,day,hour) select * from temp_GantryBaseInfoData")
        // offset更新
