...
import com.cmnit.gatherdata.modules.bean.GantryDownloadRecordInfo
...
object GantryDownloadRecordInfoModule {

  final val logger = Logger.getLogger(GantryDownloadRecordInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.GANTY_DOWNLOAD_RECORD.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[GantryDownloadRecordInfo])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_GantryDownloadRecordInfo")
        sparkSession.sql("insert into table ods.ods_etc_gantrydownloadrecordinfo partition(year,month,day) select * from temp_GantryDownloadRecordInfo")
        // offset更新
