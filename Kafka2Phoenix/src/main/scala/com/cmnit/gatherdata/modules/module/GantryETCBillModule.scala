...
import com.cmnit.gatherdata.modules.bean.GantryETCBill
...
object GantryETCBillModule {

  final val logger = Logger.getLogger(GantryETCBillModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.GBUPLOAD_ETCTU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_GantryETCBill")
        sparkSession.sql("insert into table ods.ods_etc_gantryetcbill partition(year,month,day,hour) select * from temp_GantryETCBill")
        // offset更新
