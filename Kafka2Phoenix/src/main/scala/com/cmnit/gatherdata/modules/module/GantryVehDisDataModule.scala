...
import com.cmnit.gatherdata.modules.bean.GantryVehDisData
...
object GantryVehDisDataModule {

  final val logger = Logger.getLogger(GantryVehDisDataModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.GBUPLOAD_VIU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[GantryVehDisData])
          }).map(info => {
          val calendar = Calendar.getInstance()
          val time = DateUtil.parse(info.picTime.toString.replace("T", " "), "yyyy-MM-dd HH:mm:ss")
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_GantryVehDisData")
        sparkSession.sql("insert into table ods.ods_etc_gantryvehdisdata partition(year,month,day,hour) select * from temp_GantryVehDisData")
        // offset更新
