...
import com.cmnit.gatherdata.modules.bean.GantryVehDisHourSum
...
object GantryVehDisHourSumModule {

  final val logger = Logger.getLogger(GantryVehDisHourSumModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.GBUPLOAD_VISU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[GantryVehDisHourSum])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_GantryVehDisHourSum")
        sparkSession.sql("insert into table ods.ods_etc_gantryvehdishoursum partition(year,month,day) select * from temp_GantryVehDisHourSum")
        // offset更新
