...
import com.cmnit.gatherdata.modules.bean.GantryVehDisPic
...
object GantryVehDisPicModule {

  final val logger = Logger.getLogger(GantryVehDisPicModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.GBUPLOAD_VIPU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[GantryVehDisPic])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_GantryVehDisPic")
        sparkSession.sql("insert into table ods.ods_etc_gantryvehdispic partition(year,month,day,hour) select * from temp_GantryVehDisPic")
        // offset更新
