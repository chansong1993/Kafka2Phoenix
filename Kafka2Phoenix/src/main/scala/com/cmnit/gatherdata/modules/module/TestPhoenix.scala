...
object TestPhoenix {
  ...
  final val logger = Logger.getLogger(TestPhoenix.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    //val topic = KafkaTopicEnum.TEST_PHOENIX.getTopic
    val topic = "TEST"
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.filter(x => !x.value.isEmpty)
          .map(_.value)
          //.map(JSON.parseArray)
          //.flatMap(_.toArray)
          .map(line => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TestPhoenix")
        //sparkSession.sql("insert into table default.test_202111 partition(year,month,day,hour) select * from temp_TestPhoenix")
        // offset更新
