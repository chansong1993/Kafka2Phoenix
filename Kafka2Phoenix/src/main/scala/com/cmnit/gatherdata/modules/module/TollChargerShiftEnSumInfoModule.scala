...
import com.cmnit.gatherdata.modules.bean.TollChargerShiftEnSumInfo
...
object TollChargerShiftEnSumInfoModule {

  final val logger = Logger.getLogger(TollChargerShiftEnSumInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_ENLSU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollChargerShiftEnSumInfo])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollChargerShiftEnSumInfo")
        sparkSession.sql("insert into table ods.ods_etc_tollchargershiftensuminfo partition(year,month,day) select * from temp_TollChargerShiftEnSumInfo")
        // offset更新
