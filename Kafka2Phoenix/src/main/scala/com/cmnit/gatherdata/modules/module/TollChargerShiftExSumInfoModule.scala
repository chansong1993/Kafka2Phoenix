...
import com.cmnit.gatherdata.modules.bean.TollChargerShiftExSumInfo
...
object TollChargerShiftExSumInfoModule {

  final val logger = Logger.getLogger(TollChargerShiftExSumInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_EXLSU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollChargerShiftExSumInfo])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollChargerShiftExSumInfo")
        sparkSession.sql("insert into table ods.ods_etc_tollchargershiftexsuminfo partition(year,month,day) select * from temp_TollChargerShiftExSumInfo")
        // offset更新
