...
import com.cmnit.gatherdata.modules.bean.TollEnBillInfo
...
object TollEnBillInfoModule {

  final val logger = Logger.getLogger(TollEnBillInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_ENPU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          val time = DateUtil.parse(info.enTime.toString.replace("T", " "), "yyyy-MM-dd HH:mm:ss")
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollEnBillInfo")
        sparkSession.sql("insert into table ods.ods_etc_tollenbillinfo partition(year,month,day,hour) select * from temp_TollEnBillInfo")
        // offset更新
