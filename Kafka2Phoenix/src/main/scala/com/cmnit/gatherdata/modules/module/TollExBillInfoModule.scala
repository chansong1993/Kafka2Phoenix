...
import com.cmnit.gatherdata.modules.bean.TollExBillInfo
...
object TollExBillInfoModule {

  final val logger = Logger.getLogger(TollExBillInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TOLL_ORI_OUT_TRADE_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          val time = DateUtil.parse(info.exTime.toString.replace("T", " "), "yyyy-MM-dd HH:mm:ss")
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollExBillInfo")
        sparkSession.sql("insert into table ods.ods_etc_tollexbillinfo partition(year,month,day,hour) select * from temp_TollExBillInfo")
        // offset更新
