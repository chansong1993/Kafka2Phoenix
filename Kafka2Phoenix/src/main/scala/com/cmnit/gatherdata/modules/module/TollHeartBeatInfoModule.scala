...
import com.cmnit.gatherdata.modules.bean.TollHeartBeatInfo
...
object TollHeartBeatInfoModule {

  final val logger = Logger.getLogger(TollHeartBeatInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.RM_LHBU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollHeartBeatInfo])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollHeartBeatInfo")
        sparkSession.sql("insert into table ods.ods_etc_tollheartbeatinfo partition(year,month,day,hour) select * from temp_TollHeartBeatInfo")
        // offset更新
