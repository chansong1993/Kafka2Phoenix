...
import com.cmnit.gatherdata.modules.bean.TollLoadGantryBillHourSum
...
object TollLoadGantryBillHourSumModule {

  final val logger = Logger.getLogger(TollLoadGantryBillHourSumModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_ETCSU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollLoadGantryBillHourSum])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollLoadGantryBillHourSum")
        sparkSession.sql("insert into table ods.ods_etc_tollloadgantrybillhoursum partition(year,month,day) select * from temp_TollLoadGantryBillHourSum")
        // offset更新
