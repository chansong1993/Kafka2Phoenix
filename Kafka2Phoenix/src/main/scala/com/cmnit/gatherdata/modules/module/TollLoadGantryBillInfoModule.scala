...
import com.cmnit.gatherdata.modules.bean.TollLoadGantryBillInfo
...
object TollLoadGantryBillInfoModule {

  final val logger = Logger.getLogger(TollLoadGantryBillInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_ETCTU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollLoadGantryBillInfo])
          }).map(info => {
          val calendar = Calendar.getInstance()
          val time = DateUtil.parse(info.transTime.toString.replace("T", " "), "yyyy-MM-dd HH:mm:ss")
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollLoadGantryBillInfo")
        sparkSession.sql("insert into table ods.ods_etc_tollloadgantrybillinfo partition(year,month,day,hour) select * from temp_TollLoadGantryBillInfo")
        // offset更新
