...
import com.cmnit.gatherdata.modules.bean.TollOpLogInfo
...
object TollOpLogInfoModule {

  final val logger = Logger.getLogger(TollOpLogInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_LOPLU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollOpLogInfo])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollOpLogInfo")
        sparkSession.sql("insert into table ods.ods_etc_tolloploginfo partition(year,month,day,hour) select * from temp_TollOpLogInfo")
        // offset更新
