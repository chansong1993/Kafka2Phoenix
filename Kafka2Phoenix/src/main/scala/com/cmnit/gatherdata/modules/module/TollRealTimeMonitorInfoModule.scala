...
import com.cmnit.gatherdata.modules.bean.TollRealTimeMonitorInfo
...
object TollRealTimeMonitorInfoModule {

  // TODO 修改类名
  final val logger = Logger.getLogger(TollRealTimeMonitorInfoModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    // TODO 修改topic
    val topic = KafkaTopicEnum.RM_RRU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(line => {
            // TODO 修改类型
            JSON.parseObject(line.toString, classOf[TollRealTimeMonitorInfo])
          }).map(info => {
        ...
        // 创建视图
        // TODO 修改视图名和数仓表
        df.createOrReplaceTempView("temp_TollRealTimeMonitorInfo")
        sparkSession.sql("insert into table ods.ods_etc_tollrealtimemonitorinfo partition(year,month,day,hour) select * from temp_TollRealTimeMonitorInfo")
        // offset更新
