...
import com.cmnit.gatherdata.modules.bean.TollVehDisData
...
object TollVehDisDataModule {

  final val logger = Logger.getLogger(TollVehDisDataModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_VIU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollVehDisData])
          }).map(info => {
          val calendar = Calendar.getInstance()
          val time = DateUtil.parse(info.pointTime.toString.replace("T", " "), "yyyy-MM-dd HH:mm:ss")
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollVehDisData")
        sparkSession.sql("insert into table ods.ods_etc_tollvehdisdata partition(year,month,day,hour) select * from temp_TollVehDisData")
        // offset更新
