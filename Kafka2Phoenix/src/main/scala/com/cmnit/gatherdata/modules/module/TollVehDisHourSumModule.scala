...
import com.cmnit.gatherdata.modules.bean.TollVehDisHourSum
...
object TollVehDisHourSumModule {

  // TODO 修改类名
  final val logger = Logger.getLogger(TollVehDisHourSumModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    // TODO 修改topic
    val topic = KafkaTopicEnum.TRC_TOLLDISHSU.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(line => {
            // TODO 修改类型
            JSON.parseObject(line.toString, classOf[TollVehDisHourSum])
          }).map(info => {
        ...
        // 创建视图
        // TODO 修改视图名和数仓表
        df.createOrReplaceTempView("temp_TollVehDisHourSum")
        sparkSession.sql("insert into table ods.ods_etc_tollvehdishoursum partition(year,month,day) select * from temp_TollVehDisHourSum")
        // offset更新
