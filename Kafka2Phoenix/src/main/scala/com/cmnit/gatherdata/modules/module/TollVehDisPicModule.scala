...
import com.cmnit.gatherdata.modules.bean.TollVehDisPic
...
object TollVehDisPicModule {

  final val logger = Logger.getLogger(TollVehDisPicModule.getClass)

  def startStreaming(sparkSession: SparkSession, kafkaBootstrapServers: String, zkConnect: String): Unit = {

    val topic = KafkaTopicEnum.TRC_SVIPU_TOPIC.getTopic
    ...
    dataInputStream.foreachRDD(rdd => {
      try {
        val df = rdd.repartition(2)
          ...
          .map(_.value)
          .map(line => {
            JSON.parseObject(line.toString, classOf[TollVehDisPic])
          }).map(info => {
        ...
        df.show(30, false)
        // 创建视图
        df.createOrReplaceTempView("temp_TollVehDisPic")
        sparkSession.sql("insert into table ods.ods_etc_tollvehdispic partition(year,month,day,hour) select * from temp_TollVehDisPic")
        // offset更新
